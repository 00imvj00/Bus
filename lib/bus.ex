defmodule Bus do
  ...
  # CALLBACKS
  @callback on_connect() :: :ok
  @callback on_disconnect() :: :ok
  @callback on_message(binary) :: :ok
  @optional_callbacks on_connect: 0, on_disconnect: 0, on_message: 1
  ...
  defmacro __using__(_opts) do
    quote do
      ...
      def on_message(_message) do
      end

      defoverridable on_connect: 0, on_disconnect: 0, on_message: 1
    end
  end

  ######### API START
  @spec start_link(any(), any()) :: :ignore | {:error, any()} | {:ok, pid()}
  def start_link(module, _args) do
    state = %{
  ...
  # Process the packet.
  defp handle_message(%{ids: id_list, module: mod} = state, message) do
    case message do
      %Message.ConnAck{} ->
